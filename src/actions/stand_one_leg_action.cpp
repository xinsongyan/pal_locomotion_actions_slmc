#include <pal_locomotion_actions/stand_one_leg_action.h>
...
bool StandOneLegAction::cycleHook(const ros::Time &time)
{
  ...
  else
  {
    ...
    {
      eMatrixHom target_foot_pose = createMatrix(parameters_.target_swing_leg_orientation_[swing_leg_pose_index_],
      ...
      ROS_INFO_STREAM("created swing leg trajectory, local target position: " << target_relative_foot_pose.translation());

      swing_leg_interpolator_->setPoseTarget(target_relative_foot_pose,ros::Duration(parameters_.swing_leg_duration_[swing_leg_pose_index_]));
      ...
      ss_time_ = internal_time_ + ros::Duration(parameters_.swing_leg_duration_[swing_leg_pose_index_]);

      ++swing_leg_pose_index_;
    }

    eMatrixHom desired_local_swing_leg_pose = swing_leg_interpolator_->getDesiredPose();
    std::pair<Eigen::Vector3d, Eigen::Vector3d> desired_local_swing_leg_velocity = swing_leg_interpolator_->getDesiredVelocity();
    std::pair<Eigen::Vector3d, Eigen::Vector3d> desired_local_swing_leg_acceleration = swing_leg_interpolator_->getDesiredAcceleration();
