...
g = 9.81
z_c = 0.88
step_duration = [1.0, 1.0]  # index 0 is double support, index 1 is single support
...
support_durations = np.array([step_duration[int(abs(support_indexes[i]))] for i in range(support_indexes.size)])
...
foot_placement[0, 0] = x_offset
foot_placement[3, 0] = x_offset
...
for i in range(support_indexes.size):
    ...
    if support_indexes[i] == -1:  # right support
        ...
        foot_placement[0, i] = com_x_vel * single_support_count * (np.sum(step_duration) / step_duration[0]) + x_offset
        foot_placement[1, i] = cop_y_max
        cop_ref_x[:, int(support_durations_acm[i] / dt):int(support_durations_acm[i + 1] / dt)] = foot_placement[3, i]
        cop_ref_y[:, int(support_durations_acm[i] / dt):int(support_durations_acm[i + 1] / dt)] = p_step_y
        ...
        single_support_count += 1
        foot_placement[:3, i] = foot_placement[:3, i - 1]
        foot_placement[3, i] = com_x_vel * single_support_count * (np.sum(step_duration) / step_duration[0]) + x_offset
        foot_placement[4, i] = cop_y_min
        cop_ref_x[:, int(support_durations_acm[i] / dt):int(support_durations_acm[i + 1] / dt)] = foot_placement[0, i]
        cop_ref_y[:, int(support_durations_acm[i] / dt):int(support_durations_acm[i + 1] / dt)] = p_step_y
        if i == support_indexes.size - 2:
    ...
    if support_indexes[i] == 0:
        duration_num_current_step = math.ceil(support_durations[i] / dt)
        if i != 0 and i != support_indexes.size - 1:
            # to be changed
            if support_indexes[i - 1] == -1:
                for j in range(int(duration_num_current_step + 1)):
                    cop_ref_x[:, int(support_durations_acm[i] / dt) + j] = foot_placement[3, i - 1] + (
                            foot_placement[0, i] - foot_placement[3, i - 1]) * j / duration_num_current_step
                    cop_ref_y[:, int(support_durations_acm[i] / dt) + j] = foot_placement[4, i - 1] + (
                            foot_placement[1, i] - foot_placement[4, i - 1]) * j / duration_num_current_step
            else:
                for j in range(int(duration_num_current_step + 1)):
                    cop_ref_x[:, int(support_durations_acm[i] / dt) + j] = foot_placement[0, i - 1] + (
                            foot_placement[3, i] - foot_placement[0, i - 1]) * j / duration_num_current_step
                    cop_ref_y[:, int(support_durations_acm[i] / dt) + j] = foot_placement[1, i - 1] + (
            ...
            if support_indexes[i - 1] == -1:
                for j in range(int(duration_num_current_step + 1)):
                    cop_ref_x[:, int(support_durations_acm[i] / dt) + j] = foot_placement[3, i - 1] + (
                            foot_placement[0, i] - foot_placement[3, i - 1]) * j / duration_num_current_step
                    cop_ref_y[:, int(support_durations_acm[i] / dt) + j] = foot_placement[4, i - 1] + (
                            foot_placement[1, i - 1] - 0.0) * j / duration_num_current_step
            else:
                for j in range(int(duration_num_current_step + 1)):
                    cop_ref_x[:, int(support_durations_acm[i] / dt) + j] = foot_placement[0, i - 1] + (
                            foot_placement[3, i] - foot_placement[0, i - 1]) * j / duration_num_current_step
                    cop_ref_y[:, int(support_durations_acm[i] / dt) + j] = foot_placement[1, i - 1] + (
                            foot_placement[4, i - 1] - 0.0) * j / duration_num_current_step
        if i == 0:
            if support_indexes[i + 1] == -1:
                for j in range(int(duration_num_current_step + 1)):
                    cop_ref_x[:, int(support_durations_acm[i] / dt) + j] = x_offset
                    cop_ref_y[:, int(support_durations_acm[i] / dt) + j] = 0.0 + (
                            foot_placement[4, i] - 0.0) * j / duration_num_current_step

            if support_indexes[i + 1] == 1:
                for j in range(int(duration_num_current_step + 1)):
                    cop_ref_x[:, int(support_durations_acm[i] / dt) + j] = x_offset
                    cop_ref_y[:, int(support_durations_acm[i] / dt) + j] = 0.0 + (
                            foot_placement[1, i] - 0.0) * j / duration_num_current_step

# foot_placement[2, 1] = stair_height
# foot_placement[5, 1] = 0.0
# foot_placement[2, 2] = stair_height
# foot_placement[5, 2] = 0.0
# foot_placement[2, 3] = stair_height
# foot_placement[5, 3] = stair_height
# foot_placement[2, 4] = stair_height
# foot_placement[5, 4] = stair_height
...
# --- Generate Z direction motion --- #
vel_z = stair_height / T
# state_sample_z[0, :] = z_c*np.ones(sample_num)
for i in range(sample_num):
    state_sample_z[0, i] = z_c + vel_z * timetime[i]
state_sample_z[1, :] = vel_z * np.ones(sample_num)
...
com_trajectory = np.asarray(
    (timetime, state_sample_x[0, :], state_sample_y[0, :], state_sample_z[0, :], state_sample_x[1, :],
     state_sample_y[1, :], state_sample_z[1, :], state_sample_x[2, :], state_sample_y[2, :], state_sample_z[2, :])).T

# np.savetxt('com_trajectory.csv', com_trajectory, delimiter=',')

# np.savetxt('support_durations.csv', support_durations, delimiter=',')

# np.savetxt('support_indexes.csv', support_indexes, delimiter=',')

# np.savetxt('foot_placement.csv', foot_placement.T, delimiter=',')
zmp_trajectory = np.asarray((timetime, zmp_sample[0, :], zmp_sample[1, :], zmp_sample[2, :])).T
# np.savetxt('zmp_trajectory.csv', zmp_trajectory, delimiter=',')
...
if plot:
    ...
    plt.plot(time, sol.value(y), 'r-', label='com_y')
    plt.plot(time, sol.value(yd), 'g-', label='com_yd')
    plt.plot(time, sol.value(ydd), 'b-', label='com_ydd')
    ...
    # plt.figure(3)
    # plt.plot(timetime, state_sample_z[0, :], 'b--', label='sample_com_z')
    # plt.legend(loc='best')
    # plt.xlabel('time (s)')
    # plt.ylabel('pos (m)')
    # plt.title("CoM Z")
...
trajectory_publisher.publish_all(com_trajectory, zmp_trajectory, support_durations, support_indexes, foot_placement.T,
                                 swing_height, com_gain)
